...
if __name__ == '__main__':
    ...
    parser.add_argument("--event", type=str, default="./data/test1.json", help="path to event of one cart")
    parser.add_argument("--meta", type=str, default="./data/meta.json", help="path to meta.json")

    opt = parser.parse_args()

    with open(opt.event) as jFile:
        jeString = jFile.read()
    ...
    try:
        ...
        data = pd.DataFrame.from_dict(jeData)
    data = data.dropna()
    ...
    columns = ["sessionid", "sum_products", "sum_price", "morning", "afternoo", "evening", "night"]
    ...
    dataset.drop(columns=["sessionid"], inplace=True)
    payload = {'data': json.dumps(np.array(dataset).tolist())}
    recommendations = requests.post('http://127.0.0.1:5000/recommendation', data=payload).json()
    recommendations = np.array(recommendations)
    print("Related Product Recommendations:")
    print("-------------------------------")
